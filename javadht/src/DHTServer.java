...
/**
 * 每个DHTServer对应多个本地DHT节点, 每个本地DHT节点监听一个端口, 
 * 每个DHTServer对象都有一个工作线程worker, 这个线程负责当前DHTServer对象维护的所有节点的数据读写及逻辑处理, 
 * 这些操作都是非阻塞的, 因此在多核处理器上创建和处理器数量相同的DHTServer对象就能最大限度利用CPU资源
 * 每个DHTServer之间都是相互独立的, 不能存在数据共享和争用, 由于每个DHTServer内部的所有操作都是单线程, 所以所有操作都不需要进行线程同步, 最大限度消除同步控制的开销
 * @author dgqjava
 *
 */
public class DHTServer {
    private static final NodeInfo[] ROOT_NODES; // DHT根节点, 这是几个长期稳定的公用节点, 通过这几个节点查找其他节点来初始化路由表
    
    private final List<LocalDHTNode> localDHTNodes = new ArrayList<>(); // 这里面是当前DHTServer对象管理的本地的DHT节点
    private final ScheduledExecutorService worker = Executors.newSingleThreadScheduledExecutor(); // 这个线程负责处理所有的操作, 包括 : NIO数据读写, 定时处理外部节点发送过来的请求数据并响应, 定时发起查找find_node请求来更新本地路由表及让更多的其他节点认识我们
    private final Map<String, LocalDHTNode> id2LocalDHTNode = new HashMap<>(); // 本地的nodeId和本地DHT节点的映射表
    private final NIOHelper nioHelper = new NIOHelper(worker); // NIOHelper类里封装了所有NIO操作, 这样在DHTServer里只需要负责处理其他节点的请求数据和发送find_node等业务处理的逻辑而不需要关注NIO相关的所有代码
    static {
    	try {
            ...
            public void run() {
                try {
                	...
                	while(!nioHelper.getReadDataQueue().isEmpty()) {
	                    ...
	                    if(y.equals("r")) {
	                    	// 获取数据来源方的nodeId
	                    	String remoteId = new String(((BencodeMap) (response.get(new BencodeString("r")))).get(new BencodeString("id")).getData(), "iso-8859-1");

		                    // 将请求发送方的信息保存到路由表, 因为所有对我们发送请求或者响应请求的节点肯定都是活跃节点, 路由表中的桶使用LUR算法实现, 直接替换掉最旧的节点
	                    	routingList.addNode(new NodeInfo(remoteAddress.getAddress().getHostAddress(), remoteAddress.getPort(), remoteId));
	                    ...
	                    if(y.equals("q")) {
	                    	// 获取数据来源方的nodeId
	                        String remoteId = new String(((BencodeMap) (response.get(new BencodeString("a")))).get(new BencodeString("id")).getData(), "iso-8859-1");
	                        ...
	                        if(q.equals("find_node")) {
	                            ...
	                            responseFindNode(readData.getId(), remoteAddress, t, routingList.getNearestNodes(new NodeId(target.getBytes("iso-8859-1"))));
	                            continue;
	                        }
	                        
	                        // 处理get_peers请求, 这个是对方向我们查询可以下载到指定磁力链接数据的节点信息, 如果我们没有可下载的节点则提供最近的8个节点返回, 因为我们不保存peer的信息, 所以我们这里每次都只返回最近的八个节点
	                        if(q.equals("get_peers")) {
	                            ...
	                            responseGetPeers(readData.getId(), remoteAddress, t, routingList.getNearestNodes(new NodeId(infoHash.getBytes("iso-8859-1"))));
	                            continue;
	                        }
	                        
	                        // 处理announce_peer请求, 这个是对方告诉我们他已经找到了文件的下载地址, 我们这里只获取他正在下载的磁力链接, 这里获取的磁力链接基本上是有效的
            ...
            public void run() {
            	try {
	                ...
	                for(LocalDHTNode localDHTNode : localDHTNodes) {
	                    ...
	                    oldNodes.add(new String(new byte[] {ipData[0], ipData[1], ipData[2], ipData[3], portData[0], portData[1]}, "iso-8859-1"));
    ...
    public void findNode(String id, InetSocketAddress target, String targetId) {
    	try {
	        ...
	        // 请求数据放入到队列中等待异步发送
	        nioHelper.write(id, new NIOHelper.WriteData(DHTHelper.getFindNodeData(transactionId, id, targetId), target));
    	} catch (Exception e) {
    		throw new RuntimeException(e);
    	}
    }
    ...
    /**
     * 根据一个节点id随机获取一个目标节点id, 
     * 如果每次都只根据自身的节点id进行find_node操作很容易就会遍历完所有的节点导致无节点可遍历, 
